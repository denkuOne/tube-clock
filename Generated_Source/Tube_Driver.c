...
#include <Tube_Driver_DMA_Seg_dma.h>
    ...
    #include <Tube_Driver_DMA_BC_dma.h>
#endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */

/* Function prototypes */
static void Tube_Driver_DmaInit(void) ;
static void Tube_Driver_DmaEnable(void) ;
static void Tube_Driver_DmaDisable(void) ;

/* Used to hold the common line output sequence */
#if(Tube_Driver_COM_DRIVE == Tube_Driver_HIGH)
    static uint8 Tube_Driver_comBuffer[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u, 0x00u, 0x08u,
                                                    0x00u, 0x10u, 0x00u, 0x20u, 0x00u, 0x40u, 0x00u, 0x80u};
#else
    static uint8 Tube_Driver_comBuffer[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu, 0xFFu, 0xF7u,
                                                    0xFFu, 0xEFu, 0xFFu, 0xDFu, 0xFFu, 0xBFu, 0xFFu, 0x7Fu};
#endif /* (Tube_Driver_COM_DRIVE == Tube_Driver_HIGH) */

/* Buffers used to hold the display(segment) values to be transferred to the control registers */
uint8 Tube_Driver_segLBuffer[Tube_Driver_NUM_COM];
#if (Tube_Driver_NUM_SEG > 8u) 
    uint8 Tube_Driver_segMBuffer[Tube_Driver_NUM_COM];
#endif /* (Tube_Driver_NUM_SEG > 8u) */
#if (Tube_Driver_NUM_SEG > 16u) 
    uint8 Tube_Driver_segHBuffer[Tube_Driver_NUM_COM];
#endif /* (Tube_Driver_NUM_SEG > 16u) */

#if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
    /* The brightness register used to hold the brightness levels for each common */
    static uint8 Tube_Driver_brightnessReg[Tube_Driver_NUM_COM];
#endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */

/* DMA channel and TD declaractions */
static uint8 Tube_Driver_dmaComChannel;
static uint8 Tube_Driver_dmaComTd[Tube_Driver_TWO_NUM_COM];

static uint8 Tube_Driver_dmaSegChannel;
static uint8 Tube_Driver_dmaSegLTd[Tube_Driver_NUM_COM];

#if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
    static uint8 Tube_Driver_dmaBCChannel;
    static uint8 Tube_Driver_dmaBCTd;
#endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */

uint8 Tube_Driver_initVar = 0u;
uint8 Tube_Driver_enableVar = 0u;


/*******************************************************************************
* Function Name: Tube_Driver_DmaInit
********************************************************************************
*
* Summary: 
...
static void Tube_Driver_DmaInit(void) 
{
    ...
    #if (Tube_Driver_NUM_SEG > 8u)
        static uint8 Tube_Driver_dmaSegMTd[Tube_Driver_NUM_COM];
    #endif /* (Tube_Driver_NUM_SEG > 8u) */
    #if (Tube_Driver_NUM_SEG > 16u)
        static uint8 Tube_Driver_dmaSegHTd[Tube_Driver_NUM_COM];
    #endif /* (Tube_Driver_NUM_SEG > 16u) */

    /************************* DMA configuration for the Commons *************************/
    /* Common DMA channel */
    Tube_Driver_dmaComChannel = Tube_Driver_DMA_Com_DmaInitialize(1u, 1u, 
                                        HI16(Tube_Driver_SRAM_BASE), HI16(Tube_Driver_CONTROL_REG_BASE));

    /* Check if the channel is valid */                                                                        
    if(Tube_Driver_dmaComChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate the common TDs */
        for(i = 0u; i < Tube_Driver_TWO_NUM_COM; i++)
        {
            Tube_Driver_dmaComTd[i] = CyDmaTdAllocate();
        }
        
        /* Set configuration for the Common TDs */
        for(i = 0u;  i < (Tube_Driver_TWO_NUM_COM - 1u); i++)
        {
            /* Separate configurations for odd and even TDs */
            if((uint8)(i & 1u) == 0u)
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(Tube_Driver_dmaComTd[i], 1u, 
                                                Tube_Driver_dmaComTd[i+1u],
                                                TD_INC_SRC_ADR | Tube_Driver_DMA_Com__TD_TERMOUT_EN);
            }
            else
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(Tube_Driver_dmaComTd[i], 1u,
                                                Tube_Driver_dmaComTd[i+1u], TD_INC_SRC_ADR);
            }
        }
        /* Set the wraparound Common TD configuration */
        (void) CyDmaTdSetConfiguration(Tube_Driver_dmaComTd[Tube_Driver_TWO_NUM_COM - 1u], 
                                        1u, Tube_Driver_dmaComTd[0u], TD_INC_SRC_ADR);
        
        /* Set the lower addresses of the common TDs */
        for(i = 0u; i < Tube_Driver_TWO_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(Tube_Driver_dmaComTd[i],
                                        LO16((uint32) &Tube_Driver_comBuffer[i]),
                                        LO16((uint32) Tube_Driver_COM_CONTROL_PTR));
        }    
    }
    ...
    Tube_Driver_dmaSegChannel = Tube_Driver_DMA_Seg_DmaInitialize(1u, 1u, 
                                        HI16(Tube_Driver_SRAM_BASE), HI16(Tube_Driver_CONTROL_REG_BASE));
    
    /* Check if the segment channel is valid */
    if(Tube_Driver_dmaSegChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate separate Tds for segments to implement the chain */
        for(i = 0u; i < Tube_Driver_NUM_COM; i++)
        {
            Tube_Driver_dmaSegLTd[i] = CyDmaTdAllocate();
            #if (Tube_Driver_NUM_SEG > 8u)
                Tube_Driver_dmaSegMTd[i] = CyDmaTdAllocate();
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
            #if (Tube_Driver_NUM_SEG > 16u)
                Tube_Driver_dmaSegHTd[i] = CyDmaTdAllocate();
            #endif /* (Tube_Driver_NUM_SEG > 16u) */
        }
        
        /* Set the configuration for the Segment TDs */
        for(i = 0u; i < (Tube_Driver_NUM_COM - 1u); i++)
        {
            #if (Tube_Driver_NUM_SEG <= 8u)
                (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegLTd[i], 1u, Tube_Driver_dmaSegLTd[i+1u],
                                                TD_INC_SRC_ADR | Tube_Driver_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegLTd[i], 1u, Tube_Driver_dmaSegMTd[i],
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);

                #if (Tube_Driver_NUM_SEG <= 16u)
                    (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegMTd[i], 1u, Tube_Driver_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | Tube_Driver_DMA_Seg__TD_TERMOUT_EN);
                #else    
                    (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegMTd[i], 1u, Tube_Driver_dmaSegHTd[i],
                                                    TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                    (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegHTd[i], 1u, Tube_Driver_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | Tube_Driver_DMA_Seg__TD_TERMOUT_EN);
                #endif /* Tube_Driver_NUM_SEG <= 16u */
            #endif /* Tube_Driver_NUM_SEG <= 8u */
        }
        ...
        #if (Tube_Driver_NUM_SEG <= 8u)
            (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegLTd[Tube_Driver_NUM_COM - 1u],
                                            1u, Tube_Driver_dmaSegLTd[0u], 
                                            TD_INC_SRC_ADR | Tube_Driver_DMA_Seg__TD_TERMOUT_EN);
        #else    
            (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegLTd[Tube_Driver_NUM_COM - 1u],
                                            1u, Tube_Driver_dmaSegMTd[Tube_Driver_NUM_COM - 1u],
                                            TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
            #if (Tube_Driver_NUM_SEG <= 16u)
                (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegMTd[Tube_Driver_NUM_COM - 1u],
                                                1u, Tube_Driver_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | Tube_Driver_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegMTd[Tube_Driver_NUM_COM - 1u],
                                                1u, Tube_Driver_dmaSegHTd[Tube_Driver_NUM_COM - 1u], 
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                (void) CyDmaTdSetConfiguration(Tube_Driver_dmaSegHTd[Tube_Driver_NUM_COM - 1u],
                                                1u, Tube_Driver_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | Tube_Driver_DMA_Seg__TD_TERMOUT_EN);
            #endif /* (Tube_Driver_NUM_SEG <= 16u) */
        #endif /* (Tube_Driver_NUM_SEG <= 8u) */
        ...
        for(i = 0u; i < Tube_Driver_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(Tube_Driver_dmaSegLTd[i],
                                            LO16((uint32) &Tube_Driver_segLBuffer[i]),
                                            LO16((uint32) Tube_Driver_SEG_L_CONTROL_PTR));
            #if (Tube_Driver_NUM_SEG > 8u)
                (void) CyDmaTdSetAddress(Tube_Driver_dmaSegMTd[i],
                                            LO16((uint32) &Tube_Driver_segMBuffer[i]),
                                            LO16((uint32) Tube_Driver_SEG_M_CONTROL_PTR));
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
            #if (Tube_Driver_NUM_SEG > 16u)
                (void) CyDmaTdSetAddress(Tube_Driver_dmaSegHTd[i], 
                                            LO16((uint32) &Tube_Driver_segHBuffer[i]),
                                            LO16((uint32) Tube_Driver_SEG_H_CONTROL_PTR));
            #endif /* (Tube_Driver_NUM_SEG > 16u) */
        }
    }
    ...
    /******************** DMA configuration for Brightness control *********************/
        ...
        Tube_Driver_dmaBCChannel = Tube_Driver_DMA_BC_DmaInitialize(1u, 1u, 
                                        HI16(Tube_Driver_SRAM_BASE), HI16(Tube_Driver_CONTROL_REG_BASE));
            
        /* Check if the channels are valid */
        if(Tube_Driver_dmaBCChannel != DMA_INVALID_CHANNEL)
        {
            Tube_Driver_dmaBCTd = CyDmaTdAllocate();
            
            /* Set the TD configuration for the brightness DMA */
            (void) CyDmaTdSetConfiguration(Tube_Driver_dmaBCTd, Tube_Driver_NUM_COM,
                                        Tube_Driver_dmaBCTd, TD_INC_SRC_ADR);
            
            /* Lower address for the brightness Td */
            (void) CyDmaTdSetAddress(Tube_Driver_dmaBCTd, LO16((uint32) &Tube_Driver_brightnessReg[0u]),
                                                               LO16((uint32)Tube_Driver_COMPARE_PTR));
        }
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */
}
...
/*******************************************************************************
* Function Name: Tube_Driver_DmaEnable
********************************************************************************
*
* Summary: 
...
static void Tube_Driver_DmaEnable(void) 
{
    ...
    (void) CyDmaChSetInitialTd(Tube_Driver_dmaComChannel, Tube_Driver_dmaComTd[0u]);
    (void) CyDmaChSetInitialTd(Tube_Driver_dmaSegChannel, Tube_Driver_dmaSegLTd[0u]);
    #if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
        (void) CyDmaChSetInitialTd(Tube_Driver_dmaBCChannel, Tube_Driver_dmaBCTd);
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */
    
    /* Enable all channels */
    (void) CyDmaChEnable(Tube_Driver_dmaComChannel, 1u);
    (void) CyDmaChEnable(Tube_Driver_dmaSegChannel, 1u);
    #if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
        (void) CyDmaChEnable(Tube_Driver_dmaBCChannel, 1u);
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */
}


/*******************************************************************************
* Function Name: Tube_Driver_DmaDisable
********************************************************************************
*
* Summary: 
...
static void Tube_Driver_DmaDisable(void) 
{
    uint8 i;
    uint8 channel;
    uint8 dmaChannelList[Tube_Driver_CHANNEL_LIMIT];
    
    dmaChannelList[0u] = Tube_Driver_dmaComChannel;
    dmaChannelList[1u] = Tube_Driver_dmaSegChannel;
    #if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
        dmaChannelList[2u] = Tube_Driver_dmaBCChannel;
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */

    for(i = 0u; i < Tube_Driver_CHANNEL_LIMIT; i++)
    {
        channel = dmaChannelList[i];
        (void) CyDmaChDisable(channel);
        
        Tube_Driver_Com_Driver_Write(Tube_Driver_CLEAR);
...
/*******************************************************************************
* Function Name: Tube_Driver_Init
********************************************************************************
*
* Summary: 
...
{
    static uint8 Tube_Driver_dmaInitDone = 0u;
    #if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
        uint8 i;
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */
    ...
    /* Initialize the DMAs */
    if(0u == Tube_Driver_dmaInitDone)
    {
        Tube_Driver_DmaInit();
        Tube_Driver_dmaInitDone = 1u;
    }
    ...
    /* Set the brightness to full */
        ...
        for(i = 0u; i<(Tube_Driver_NUM_COM); i++)
        {
            Tube_Driver_brightnessReg[i] = Tube_Driver_INIT_BRIGHT;
        }
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */
}
...
/*******************************************************************************
* Function Name: Tube_Driver_Enable
********************************************************************************
*
* Summary: 
...
{
    /* Enable the DMAs */
    Tube_Driver_DmaEnable();
 
    #if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
       /* Initialize to zero */
       Tube_Driver_COUNTER_REG = 0x00u;
       Tube_Driver_COMPARE_REG = 0x00u;
       
        /* Enable the PWM for brightness control */
       Tube_Driver_CONTROL_REG = Tube_Driver_CTRL_ENABLE;
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */
    
    Tube_Driver_enableVar = Tube_Driver_CTRL_ENABLE;
}


/*******************************************************************************
* Function Name: Tube_Driver_Start
********************************************************************************
*
* Summary:
...
{
    if(0u == Tube_Driver_initVar)
    {
        Tube_Driver_Init();
        Tube_Driver_initVar = 1u;
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_Stop
********************************************************************************
*
* Summary:
...
void Tube_Driver_Stop(void) 
{
    #if (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED)
        /* Disable the PWM */
        Tube_Driver_CONTROL_REG = Tube_Driver_CTRL_DISABLE;
    #endif /* (Tube_Driver_BRIGHTNESS_EN == Tube_Driver_ENABLED) */
    
    Tube_Driver_DmaDisable();
    Tube_Driver_ClearDisplayAll();
    
    Tube_Driver_enableVar  = Tube_Driver_CTRL_DISABLE;
}


/*******************************************************************************
* Function Name: Tube_Driver_SetDisplayRAM
********************************************************************************
*
* Summary:
...
*  RAM directly by writing to Tube_Driver_segLBuffer,
*  Tube_Driver_segMBuffer, Tube_Driver_segHBuffer.
*
* Parameters:
...
void Tube_Driver_SetDisplayRAM(uint8 value, uint8 position) 
{
    ...
    if(position < Tube_Driver_NUM_COM)
    {
        relPosition = position;
        #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
            Tube_Driver_segLBuffer[relPosition] = value;
        #else
            Tube_Driver_segLBuffer[relPosition] = ((uint8) ~value);
        #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
    }    
    else if (position < Tube_Driver_TWO_NUM_COM)
    {
        #if (Tube_Driver_NUM_SEG > 8u)
            relPosition = position - Tube_Driver_NUM_COM;
            #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                Tube_Driver_segMBuffer[relPosition] = value;
            #else
                Tube_Driver_segMBuffer[relPosition] = ((uint8) ~value);
            #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
    }
    else
    {
        #if (Tube_Driver_NUM_SEG > 16u)
            relPosition = position - Tube_Driver_TWO_NUM_COM;
            if(relPosition < Tube_Driver_NUM_COM)
            {
                #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                    Tube_Driver_segHBuffer[relPosition] = value;
                #else
                    Tube_Driver_segMBuffer[relPosition] = ((uint8) ~value);
                #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
            }
...
/*******************************************************************************
* Function Name: Tube_Driver_SetRC
********************************************************************************
*
* Summary:
...
void Tube_Driver_SetRC(uint8 row, uint8 column) 
{
    ...
    if(column < Tube_Driver_NUM_COM)
    {
        if(row < 8u)
        {
            rowSeg = row;
            #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                Tube_Driver_segLBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
            #else
                Tube_Driver_segLBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
            #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
        }
        else if (row < 16u)
        {
                ...
                rowSeg = row - 8u;
                #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                    Tube_Driver_segMBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    Tube_Driver_segMBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSeg = row - 16u;
                #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                    Tube_Driver_segHBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    Tube_Driver_segHBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
            #endif /* (Tube_Driver_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_ClearRC
********************************************************************************
*
* Summary:
...
void Tube_Driver_ClearRC(uint8 row, uint8 column) 
{
    ...
    if(column < Tube_Driver_NUM_COM)
    {
        if(row < 8u)
        {
            ...
            #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                Tube_Driver_segLBuffer[column] &= ((uint8)(~(uint8)rowSelect));
            #else
                Tube_Driver_segLBuffer[column] |= rowSelect;
            #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
        }    
        else if (row < 16u)
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                    Tube_Driver_segMBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    Tube_Driver_segMBuffer[column] |= rowSelect;
                #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                    Tube_Driver_segHBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    Tube_Driver_segHBuffer[column] |= rowSelect;
                #endif /* Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH */
            #endif /* (Tube_Driver_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_ToggleRC
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Tube_Driver_GetRC
********************************************************************************
*
* Summary:
...
uint8 Tube_Driver_GetRC(uint8 row, uint8 column) 
{
    ...
    if(column < Tube_Driver_NUM_COM)
    {
        ...
        {
            rowSeg = row;
            rowSegSelect = (uint8)((uint8)Tube_Driver_segLBuffer[column] >> rowSeg);
        }    
        else if (row < 16u)
        {
            #if (Tube_Driver_NUM_SEG > 8u) 
                rowSeg = row - 8u;
                rowSegSelect = (uint8)((uint8)Tube_Driver_segMBuffer[column] >> rowSeg);
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
        }
        else
        {
            #if (Tube_Driver_NUM_SEG > 16u)
                rowSeg = row - 16u;
                rowSegSelect = (uint8)((uint8)Tube_Driver_segHBuffer[column] >> rowSeg);
            #endif /* (Tube_Driver_NUM_SEG > 16u) */
        }
...
/*******************************************************************************
* Function Name: Tube_Driver_ClearDisplay
********************************************************************************
*
* Summary:
...
{  
    ...
    {
        relPosition = position;
        Tube_Driver_segLBuffer[relPosition] = Tube_Driver_CLEAR;
    }    
    else if (position < Tube_Driver_TWO_NUM_COM)
    {
        #if (Tube_Driver_NUM_SEG > 8u)
            relPosition = position - Tube_Driver_NUM_COM;
            Tube_Driver_segMBuffer[relPosition] = Tube_Driver_CLEAR;
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < Tube_Driver_NUM_COM)
            {
                Tube_Driver_segHBuffer[relPosition] = Tube_Driver_CLEAR;
            }
        #endif /* (Tube_Driver_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: Tube_Driver_ClearDisplayAll
********************************************************************************
*
* Summary:
...
void Tube_Driver_ClearDisplayAll(void) 
{
    ...
    for(position = 0u; position < Tube_Driver_NUM_COM; position++)
    {
        Tube_Driver_segLBuffer[position] = Tube_Driver_CLEAR; 
        
        #if (Tube_Driver_NUM_SEG > 8u) 
            Tube_Driver_segMBuffer[position] = Tube_Driver_CLEAR;
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
        
        #if (Tube_Driver_NUM_SEG > 16u)
            Tube_Driver_segHBuffer[position] = Tube_Driver_CLEAR;
        #endif /* (Tube_Driver_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: Tube_Driver_Write7SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < Tube_Driver_NUM_COM)
    {
        choiceReg = Tube_Driver_SEG_REG_LOW;
        relPosition = position;
    }
    else if (position < Tube_Driver_TWO_NUM_COM)
    {
        choiceReg = Tube_Driver_SEG_REG_MID;
        relPosition = position - Tube_Driver_NUM_COM;
    }
    else
    {
        choiceReg = Tube_Driver_SEG_REG_HIGH;
        relPosition = position - Tube_Driver_TWO_NUM_COM;
    }
    
    /* Routine to extract the sign and to convert to unsigned */
    if(number >= Tube_Driver_ZERO_NEG)
    {
        numberVal = (uint32)number;
        numberSign = Tube_Driver_UNSIGNED;
    }
    else
    {
        numberVal = (uint32)(-number);
        numberSign = Tube_Driver_SIGNED;
    }
    ...
    for(i = digits; i > 0u; i--)
    {
        ...
        bufArray[i-1u] = Tube_Driver_EncodeNumber7Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    
    /* Clean up the array and place sign if needed */
    if(alignment == Tube_Driver_ZERO_PAD)
    {
        if(numberSign == Tube_Driver_SIGNED)
        {
            /* Place sign on the MSB */
            bufArray[0u] = Tube_Driver_EncodeChar7Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != Tube_Driver_ZERO_7CHAR)
            {
                if(numberSign == Tube_Driver_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = Tube_Driver_EncodeChar7Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = Tube_Driver_EncodeChar7Seg('-');
                    }
                }
    ...
    if(alignment == Tube_Driver_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != Tube_Driver_CLEAR)
            {
                if(relPosition < Tube_Driver_NUM_COM)
                {
                    ...
                    switch(choiceReg)
                    {
                        case Tube_Driver_SEG_REG_LOW :
                            Tube_Driver_segLBuffer[relPosition] = bufArray[i];
                            break;
                        case Tube_Driver_SEG_REG_MID :
                            #if (Tube_Driver_NUM_SEG > 8u)
                                Tube_Driver_segMBuffer[relPosition] = bufArray[i];
                            #endif /* (Tube_Driver_NUM_SEG > 8u) */
                            break;
                        case Tube_Driver_SEG_REG_HIGH :
                            #if (Tube_Driver_NUM_SEG > 16u)
                                Tube_Driver_segHBuffer[relPosition] = bufArray[i];
                            #endif /* (Tube_Driver_NUM_SEG > 16u) */
                            break;
                        default :
                            Tube_Driver_segLBuffer[relPosition] = bufArray[i];
    ...
    else
    {
        ...
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < Tube_Driver_NUM_COM)
            {
                /* Decide which row to use */
                switch(choiceReg)
                {
                    case Tube_Driver_SEG_REG_LOW :
                        Tube_Driver_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        break;
                    case Tube_Driver_SEG_REG_MID :
                        #if (Tube_Driver_NUM_SEG > 8u)
                            Tube_Driver_segMBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (Tube_Driver_NUM_SEG > 8u) */
                        break;
                    case Tube_Driver_SEG_REG_HIGH :
                        #if (Tube_Driver_NUM_SEG > 16u)
                            Tube_Driver_segHBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (Tube_Driver_NUM_SEG > 16u) */
                        break;
                    default :
                        Tube_Driver_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
...
/*******************************************************************************
* Function Name: Tube_Driver_Write7SegNumberHex
********************************************************************************
*
* Summary:
...

{
    ...
    for(i = digits; i > 0u; i--)
    {
        selectVal = (uint8)((uint8)numberVal & Tube_Driver_NIBBLE_MASK);
        bufArray[i-1u] = Tube_Driver_EncodeNumber7Seg(selectVal);
        numberVal = (uint32)((uint32)numberVal >> Tube_Driver_SHIFT_NIBBLE);
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_WriteString7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < Tube_Driver_NUM_COM)
    {
        ...
        for(relPosition = position; relPosition < Tube_Driver_NUM_COM; relPosition++)
        {
            if(relPosition < sizeLimit)
            {
                Tube_Driver_segLBuffer[relPosition] = Tube_Driver_EncodeChar7Seg(character[i]);
                i++;
            }
        }
    }    
    ...
    {
        #if (Tube_Driver_NUM_SEG > 8u) 
            sizeLimit = ((uint8)(strlen(character)) + position) - Tube_Driver_NUM_COM;
            for(relPosition = position - Tube_Driver_NUM_COM;
                relPosition < Tube_Driver_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    Tube_Driver_segMBuffer[relPosition] = Tube_Driver_EncodeChar7Seg(character[i]);
                    i++;
                }
            }
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
    ...
    {
        #if (Tube_Driver_NUM_SEG > 16u)
            sizeLimit = ((uint8)(strlen(character)) + position) - Tube_Driver_TWO_NUM_COM;
            for(relPosition = position - Tube_Driver_TWO_NUM_COM;
                relPosition < Tube_Driver_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    Tube_Driver_segHBuffer[relPosition] = Tube_Driver_EncodeChar7Seg(character[i]);
                    i++;
                }    
            }
        #endif /* (Tube_Driver_NUM_SEG > 16u) */
...
/*******************************************************************************
* Function Name: Tube_Driver_PutChar7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    {
        relPosition = position;
        Tube_Driver_segLBuffer[relPosition] = Tube_Driver_EncodeChar7Seg(character);
    }    
    else if (position < Tube_Driver_TWO_NUM_COM)
    {
        #if (Tube_Driver_NUM_SEG > 8u) 
            relPosition = position - Tube_Driver_NUM_COM;
            Tube_Driver_segMBuffer[relPosition] = Tube_Driver_EncodeChar7Seg(character);
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < Tube_Driver_NUM_COM)
            {
                Tube_Driver_segHBuffer[relPosition] = Tube_Driver_EncodeChar7Seg(character);
            }
        #endif /* (Tube_Driver_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: Tube_Driver_Write7SegDigitDec
********************************************************************************
*
* Summary:
...

{
    ...
    if (digit < 10u)
    {
        ...
        {
            relPosition = position;
            Tube_Driver_segLBuffer[relPosition] = Tube_Driver_EncodeNumber7Seg(digit);
        }    
        else if (position < Tube_Driver_TWO_NUM_COM)
        {
            #if (Tube_Driver_NUM_SEG > 8u)
                relPosition = position - Tube_Driver_NUM_COM;
                Tube_Driver_segMBuffer[relPosition] = Tube_Driver_EncodeNumber7Seg(digit);
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
        } 
        else
        {
            #if (Tube_Driver_NUM_SEG > 16u)
                relPosition = position - Tube_Driver_TWO_NUM_COM;
                if(relPosition < Tube_Driver_TWO_NUM_COM)
                {
                    Tube_Driver_segHBuffer[relPosition] = Tube_Driver_EncodeNumber7Seg(digit);
                }
            #endif /* (Tube_Driver_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_Write7SegDigitHex
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Tube_Driver_Write14SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = Tube_Driver_EncodeNumber14Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == Tube_Driver_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = Tube_Driver_EncodeChar14Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != Tube_Driver_ZERO_14CHAR)
            {
                if(numberSign == Tube_Driver_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = Tube_Driver_EncodeChar14Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = Tube_Driver_EncodeChar14Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
            ...
            else
            {
                ...
                {
                    /* Turn off the display */
                    bufArray[i] = Tube_Driver_CLEAR_16;
                    zeroCount++;
                }
            }    
        }
    ...
    if(alignment == Tube_Driver_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != Tube_Driver_CLEAR_16)
            {
                if(relPosition < Tube_Driver_NUM_COM)
                {
                    /* Only show displays that are not empty */
                    Tube_Driver_segLBuffer[relPosition] = LO8(bufArray[i]);
                #if (Tube_Driver_NUM_SEG > 8u)
                    Tube_Driver_segMBuffer[relPosition] = HI8(bufArray[i]);
                #endif /* (Tube_Driver_NUM_SEG > 8u) */
                }
                relPosition++;
            }
    ...
    else
    {
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < Tube_Driver_NUM_COM)
            {
                Tube_Driver_segLBuffer[(i + relPosition) - 1u] = LO8(bufArray[i-1u]);
            #if (Tube_Driver_NUM_SEG > 8u)
                Tube_Driver_segMBuffer[(i + relPosition) - 1u] = HI8(bufArray[i-1u]);
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
            }
        }  
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_Write14SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: Tube_Driver_WriteString14Seg
********************************************************************************
*
* Summary:
...

{
    ...
    for(currPosition = position; currPosition < Tube_Driver_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = Tube_Driver_EncodeChar14Seg(character[i]);
            Tube_Driver_segLBuffer[currPosition] = LO8(charDisplay);
            #if (Tube_Driver_NUM_SEG > 8u)
            Tube_Driver_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
            i++;
        }
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_PutChar14Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display
...

{
    ...
    if(position < Tube_Driver_NUM_COM)
    {
        charDisplay = Tube_Driver_EncodeChar14Seg(character);
        Tube_Driver_segLBuffer[position] = LO8(charDisplay);
    #if (Tube_Driver_NUM_SEG > 8u)
        Tube_Driver_segMBuffer[position] = HI8(charDisplay);
    #endif /* (Tube_Driver_NUM_SEG > 8u) */
    }
}
...
/*******************************************************************************
* Function Name: Tube_Driver_Write14SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < Tube_Driver_NUM_COM))
    {
        charDisplay = Tube_Driver_EncodeNumber14Seg(digit);
        Tube_Driver_segLBuffer[position] = LO8(charDisplay);
    #if (Tube_Driver_NUM_SEG > 8u)
        Tube_Driver_segMBuffer[position] = HI8(charDisplay);
    #endif /* (Tube_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Tube_Driver_Write14SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: Tube_Driver_Write16SegNumberDec
********************************************************************************
*
* Summary:
*  Displays a signed integer up to 8 characters long, starting at 'position' 
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = Tube_Driver_EncodeNumber16Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == Tube_Driver_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = Tube_Driver_EncodeChar16Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != Tube_Driver_ZERO_16CHAR)
            {
                if(numberSign == Tube_Driver_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = Tube_Driver_EncodeChar16Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = Tube_Driver_EncodeChar16Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
...
/*******************************************************************************
* Function Name: Tube_Driver_Write16SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: Tube_Driver_WriteString16Seg
********************************************************************************
*
* Summary:
*  Displays a null terminated string starting at 'position' and ending at either
...

{
    ...
    for(currPosition = position; currPosition < Tube_Driver_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = Tube_Driver_EncodeChar16Seg(character[i]);
            Tube_Driver_segLBuffer[currPosition] = LO8(charDisplay);
            #if (Tube_Driver_NUM_SEG > 8u)
                Tube_Driver_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (Tube_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Tube_Driver_PutChar16Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display 
...

{
    ...
    if(position < Tube_Driver_NUM_COM)
    {
        charDisplay = Tube_Driver_EncodeChar16Seg(character);
        Tube_Driver_segLBuffer[position] = LO8(charDisplay);
        #if (Tube_Driver_NUM_SEG > 8u)
            Tube_Driver_segMBuffer[position] = HI8(charDisplay);
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Tube_Driver_Write16SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < Tube_Driver_NUM_COM))
    {
        charDisplay = Tube_Driver_EncodeNumber16Seg(digit);
        Tube_Driver_segLBuffer[position] = LO8(charDisplay);
        #if (Tube_Driver_NUM_SEG > 8u)
            Tube_Driver_segMBuffer[position] = HI8(charDisplay);
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Tube_Driver_Write16SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: Tube_Driver_PutDecimalPoint
********************************************************************************
*
* Summary:
...

{
    ...
    value = Tube_Driver_DEC_POINT_VALUE;
    ...
    if(position < Tube_Driver_NUM_COM)
    {
        ...
        {
            #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                Tube_Driver_segLBuffer[relPosition] &= ((uint8)(~(uint8)value));
            #else
                Tube_Driver_segLBuffer[relPosition] |= ((uint8)(~(uint8)value));
            #endif /* (Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH) */
        }
    ...
    else if (position < Tube_Driver_TWO_NUM_COM)
    {
            ...
            {
                #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                    Tube_Driver_segMBuffer[relPosition] &= ((uint8)(~(uint8)value));
                #else
                    Tube_Driver_segMBuffer[relPosition] |= ((uint8)(~(uint8)value));
                #endif /* (Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH) */
            }
    ...
    else
    {
            ...
            if(relPosition < Tube_Driver_NUM_COM)
            {
                ...
                {
                    #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
                        Tube_Driver_segHBuffer[relPosition] &= ((uint8)(~(uint8)value));
                    #else
                        Tube_Driver_segHBuffer[relPosition] |= ((uint8)(~(uint8)value));
                    #endif /* (Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH) */
                }
...
/*******************************************************************************
* Function Name: Tube_Driver_GetDecimalPoint
********************************************************************************
*
* Summary:
...
uint8 Tube_Driver_GetDecimalPoint(uint8 position) 
{
    ...
    {
        relPosition = position;
        decPointVal = (uint8)((uint8)(Tube_Driver_segLBuffer[relPosition]) >> Tube_Driver_DEC_POINT);
    } 
    else if (position < Tube_Driver_TWO_NUM_COM)
    {
        #if (Tube_Driver_NUM_SEG > 8u) 
            relPosition = position - Tube_Driver_NUM_COM;
            decPointVal = (uint8)((uint8)(Tube_Driver_segMBuffer[relPosition]) >> Tube_Driver_DEC_POINT);
        #endif /* (Tube_Driver_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < Tube_Driver_NUM_COM)
            {
                decPointVal = (uint8)((uint8)(Tube_Driver_segHBuffer[relPosition]) >> Tube_Driver_DEC_POINT);
            }
        #endif /* (Tube_Driver_NUM_SEG > 16u) */
    }
    
    #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
        value = (uint8)(decPointVal & Tube_Driver_DEC_POINT_MASK);
    #else
        value = (uint8)((uint8)((uint8)~decPointVal) & Tube_Driver_DEC_POINT_MASK);
    #endif /* (Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH) */
...
/*******************************************************************************
* Function Name: Tube_Driver_EncodeNumber7Seg
********************************************************************************
*
* Summary:
...
uint8 Tube_Driver_EncodeNumber7Seg(uint8 number) 
{
    ...
    #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
        static const uint8 CYCODE Tube_Driver_7SegDigits[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    #endif /*  Tube_Driver_SEG_DRIVE */

    uint8 valNum;
    uint8 valHex;
    
    valNum = (uint8)(number & Tube_Driver_NIBBLE_MASK);
    
    valHex = Tube_Driver_7SegDigits[valNum];
...
/*******************************************************************************
* Function Name: Tube_Driver_EncodeChar7Seg
********************************************************************************
*
* Summary:
...
uint8 Tube_Driver_EncodeChar7Seg(char8 input)  
{
    /* 7-segment ASCII LUT */
    #if(Tube_Driver_SEG_DRIVE == Tube_Driver_HIGH)
        static const uint8 CYCODE Tube_Driver_7SegASCII[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    valLow = (uint8)((uint8)input & Tube_Driver_NIBBLE_MASK);
    
    /* valHigh used to access the row of LUT */
    valHigh = (uint8)((uint8)input >> Tube_Driver_SHIFT_NIBBLE);
    
    /* Check if the high nibble is within valid ASCII range */
    if ((valHigh > Tube_Driver_SEG_ASCII_LIM_LO) && (valHigh < Tube_Driver_SEG_ASCII_LIM_HI))
    {
        valHigh = valHigh - Tube_Driver_SEG_ASCII_OFFSET;
        valHex = Tube_Driver_7SegASCII[valHigh][valLow];
    }
    else
    {
        valHex = Tube_Driver_CLEAR;
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_EncodeNumber14Seg
********************************************************************************
*
* Summary:
...
uint16 Tube_Driver_EncodeNumber14Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE Tube_Driver_14SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Tube_Driver_14SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)Tube_Driver_14SegDigitsHigh[valNum] << Tube_Driver_SHIFT_BYTE);
    valLow = (uint16)Tube_Driver_14SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: Tube_Driver_EncodeChar14Seg
********************************************************************************
*
* Summary:
...
uint16 Tube_Driver_EncodeChar14Seg(char8 input) 
{
        ...
        static const uint8 CYCODE Tube_Driver_14SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Tube_Driver_14SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > Tube_Driver_SEG_ASCII_LIM_LO) && (valHigh < Tube_Driver_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 14-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)Tube_Driver_14SegASCIIHigh[valHigh][valLow] << Tube_Driver_SHIFT_BYTE);
        valHexLow = (uint16)Tube_Driver_14SegASCIILow[valHigh][valLow];
        
        valHex = valHexHigh | valHexLow;
    }
    else
    {
        valHex = Tube_Driver_CLEAR_16;
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_EncodeNumber16Seg
********************************************************************************
*
* Summary:
...
uint16 Tube_Driver_EncodeNumber16Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE Tube_Driver_16SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Tube_Driver_16SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)Tube_Driver_16SegDigitsHigh[valNum] << Tube_Driver_SHIFT_BYTE);
    valLow = (uint16)Tube_Driver_16SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: Tube_Driver_EncodeChar16Seg
********************************************************************************
*
* Summary:
...
uint16 Tube_Driver_EncodeChar16Seg(char8 input) 
{
        ...
        static const uint8 CYCODE Tube_Driver_16SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Tube_Driver_16SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > Tube_Driver_SEG_ASCII_LIM_LO) && (valHigh < Tube_Driver_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 16-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)Tube_Driver_16SegASCIIHigh[valHigh][valLow] << Tube_Driver_SHIFT_BYTE);
        valHexLow = (uint16)Tube_Driver_16SegASCIILow[valHigh][valLow];
...
/*******************************************************************************
* Function Name: Tube_Driver_SetBrightness
********************************************************************************
*
* Summary:
...
{   
    #if(Tube_Driver_COM_DRIVE == Tube_Driver_HIGH)
        static const uint8 CYCODE Tube_Driver_tempCom[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u,
                                                                  0x00u, 0x08u, 0x00u, 0x10u, 0x00u, 0x20u,
                                                                  0x00u, 0x40u, 0x00u, 0x80u};
    #else
        static const uint8 CYCODE Tube_Driver_tempCom[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu,
    ...
    if(bright == 0u)
    {
        ...
        Tube_Driver_comBuffer[(uint8)((uint8)position << 1u) + 1u] = Tube_Driver_CLEAR;
    }
    else
    {
        /* Restore the commons */
        Tube_Driver_comBuffer[(uint8)((uint8)position << 1u) + 1u]
            = Tube_Driver_tempCom[(uint8)((uint8)position << 1u) + 1u];
    }
    
    /* Update the brightness register */
    Tube_Driver_brightnessReg[position] = bright;
}


/*******************************************************************************
* Function Name: Tube_Driver_GetBrightness
********************************************************************************
*
* Summary:
...
{
    return(Tube_Driver_brightnessReg[position]);
}
