/*******************************************************************************
* File Name: Feedback_ADC_theACLK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_Start(void) 
{
    /* Set the bit to enable the clock. */
    Feedback_ADC_theACLK_CLKEN |= Feedback_ADC_theACLK_CLKEN_MASK;
	Feedback_ADC_theACLK_CLKSTBY |= Feedback_ADC_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Feedback_ADC_theACLK_CLKEN &= (uint8)(~Feedback_ADC_theACLK_CLKEN_MASK);
	Feedback_ADC_theACLK_CLKSTBY &= (uint8)(~Feedback_ADC_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_StopBlock(void) 
{
    if ((Feedback_ADC_theACLK_CLKEN & Feedback_ADC_theACLK_CLKEN_MASK) != 0u)
    {
...
#if defined(Feedback_ADC_theACLK__CFG3)
        CLK_DIST_AMASK = Feedback_ADC_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Feedback_ADC_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Feedback_ADC_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Feedback_ADC_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Feedback_ADC_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Feedback_ADC_theACLK_CLKSTBY &= (uint8)(~Feedback_ADC_theACLK_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Feedback_ADC_theACLK_GetSourceRegister();
    uint16 oldDivider = Feedback_ADC_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Feedback_ADC_theACLK_CLKEN & Feedback_ADC_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Feedback_ADC_theACLK_DIV_PTR, clkDivider);
                Feedback_ADC_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Feedback_ADC_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Feedback_ADC_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Feedback_ADC_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Feedback_ADC_theACLK_DIV_PTR, clkDivider);
				Feedback_ADC_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Feedback_ADC_theACLK_GetDividerRegister(void) 
{
    return CY_GET_REG16(Feedback_ADC_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_SetModeRegister(uint8 modeBitMask) 
{
    Feedback_ADC_theACLK_MOD_SRC |= modeBitMask & (uint8)Feedback_ADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_ClearModeRegister(uint8 modeBitMask) 
{
    Feedback_ADC_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Feedback_ADC_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Feedback_ADC_theACLK_GetModeRegister(void) 
{
    return Feedback_ADC_theACLK_MOD_SRC & (uint8)(Feedback_ADC_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Feedback_ADC_theACLK_GetDividerRegister();
    uint8 oldSrc = Feedback_ADC_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Feedback_ADC_theACLK_MOD_SRC |= CYCLK_SSS;
        Feedback_ADC_theACLK_MOD_SRC =
            (Feedback_ADC_theACLK_MOD_SRC & (uint8)(~Feedback_ADC_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Feedback_ADC_theACLK_GetSourceRegister(void) 
{
    return Feedback_ADC_theACLK_MOD_SRC & Feedback_ADC_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Feedback_ADC_theACLK_SetPhaseRegister(uint8 clkPhase) 
{
    Feedback_ADC_theACLK_PHASE = clkPhase & Feedback_ADC_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Feedback_ADC_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Feedback_ADC_theACLK_GetPhaseRegister(void) 
{
    return Feedback_ADC_theACLK_PHASE & Feedback_ADC_theACLK_PHASE_MASK;
}
