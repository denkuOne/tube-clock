...
/*******************************************************************************
* Function Name: HOLDING_CLK_Start
********************************************************************************
*
* Summary:
...
void HOLDING_CLK_Start(void) 
{
    ...
    HOLDING_CLK_CLKEN |= HOLDING_CLK_CLKEN_MASK;
	HOLDING_CLK_CLKSTBY |= HOLDING_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HOLDING_CLK_Stop
********************************************************************************
*
* Summary:
...
void HOLDING_CLK_Stop(void) 
{
    ...
    HOLDING_CLK_CLKEN &= (uint8)(~HOLDING_CLK_CLKEN_MASK);
	HOLDING_CLK_CLKSTBY &= (uint8)(~HOLDING_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HOLDING_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HOLDING_CLK_CLKEN & HOLDING_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HOLDING_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HOLDING_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HOLDING_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HOLDING_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HOLDING_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HOLDING_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HOLDING_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HOLDING_CLK_GetSourceRegister();
    uint16 oldDivider = HOLDING_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HOLDING_CLK_CLKEN & HOLDING_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HOLDING_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HOLDING_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HOLDING_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: HOLDING_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HOLDING_CLK_MOD_SRC |= modeBitMask & (uint8)HOLDING_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: HOLDING_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HOLDING_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HOLDING_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: HOLDING_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HOLDING_CLK_MOD_SRC & (uint8)(HOLDING_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: HOLDING_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HOLDING_CLK_GetDividerRegister();
    uint8 oldSrc = HOLDING_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HOLDING_CLK_MOD_SRC |= CYCLK_SSS;
        HOLDING_CLK_MOD_SRC =
            (HOLDING_CLK_MOD_SRC & (uint8)(~HOLDING_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HOLDING_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HOLDING_CLK_MOD_SRC & HOLDING_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HOLDING_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HOLDING_CLK_PHASE = clkPhase & HOLDING_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HOLDING_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HOLDING_CLK_PHASE & HOLDING_CLK_PHASE_MASK;
}
