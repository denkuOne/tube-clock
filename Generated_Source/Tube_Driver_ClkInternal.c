/*******************************************************************************
* File Name: Tube_Driver_ClkInternal.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_Start
********************************************************************************
*
* Summary:
...
void Tube_Driver_ClkInternal_Start(void) 
{
    ...
    Tube_Driver_ClkInternal_CLKEN |= Tube_Driver_ClkInternal_CLKEN_MASK;
	Tube_Driver_ClkInternal_CLKSTBY |= Tube_Driver_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
void Tube_Driver_ClkInternal_Stop(void) 
{
    ...
    Tube_Driver_ClkInternal_CLKEN &= (uint8)(~Tube_Driver_ClkInternal_CLKEN_MASK);
	Tube_Driver_ClkInternal_CLKSTBY &= (uint8)(~Tube_Driver_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Tube_Driver_ClkInternal_CLKEN & Tube_Driver_ClkInternal_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Tube_Driver_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Tube_Driver_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Tube_Driver_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Tube_Driver_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Tube_Driver_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Tube_Driver_ClkInternal_GetSourceRegister();
    uint16 oldDivider = Tube_Driver_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Tube_Driver_ClkInternal_CLKEN & Tube_Driver_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Tube_Driver_ClkInternal_DIV_PTR, clkDivider);
                Tube_Driver_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Tube_Driver_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Tube_Driver_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Tube_Driver_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Tube_Driver_ClkInternal_DIV_PTR, clkDivider);
				Tube_Driver_ClkInternal_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Tube_Driver_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Tube_Driver_ClkInternal_MOD_SRC |= modeBitMask & (uint8)Tube_Driver_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Tube_Driver_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Tube_Driver_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Tube_Driver_ClkInternal_MOD_SRC & (uint8)(Tube_Driver_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Tube_Driver_ClkInternal_GetDividerRegister();
    uint8 oldSrc = Tube_Driver_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Tube_Driver_ClkInternal_MOD_SRC |= CYCLK_SSS;
        Tube_Driver_ClkInternal_MOD_SRC =
            (Tube_Driver_ClkInternal_MOD_SRC & (uint8)(~Tube_Driver_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Tube_Driver_ClkInternal_MOD_SRC & Tube_Driver_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Tube_Driver_ClkInternal_PHASE = clkPhase & Tube_Driver_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Tube_Driver_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Tube_Driver_ClkInternal_PHASE & Tube_Driver_ClkInternal_PHASE_MASK;
}
