...
********************************************************************************/
/* `#START DMA_ISR_intc` */
extern volatile uint8 dma_done_flag;
/* `#END` */
...
void DMA_ISR_Stop(void)
{
    ...
    DMA_ISR_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef DMA_ISR_INTERRUPT_INTERRUPT_CALLBACK
        DMA_ISR_Interrupt_InterruptCallback();
    #endif /* DMA_ISR_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void DMA_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)DMA_ISR__INTC_NUMBER] = address;
}
...
cyisraddress DMA_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)DMA_ISR__INTC_NUMBER];
}
...
uint8 DMA_ISR_GetState(void)
{
    ...
    return ((*DMA_ISR_INTC_SET_EN & (uint32)DMA_ISR__INTC_MASK) != 0u) ? 1u:0u;
}
